...
static TString toTString(const Era era) {
    ...
    else if( era == COLL38T15 ) str = "3.8T collision data 2015";
...
// --------------------------------------------------------------
TLegend* TkAlStyle::legend(int nEntries, const double relWidth, const bool left, const bool top) {
  double x0 = 0.;
  double x1 = 0.;
  double y0 = 0.;
  double y1 = 0.;
  if (legendheader != "") nEntries++;
...
TPaveText* TkAlStyle::title(const TString& txt) {
  ...
  if (txt.Contains("#scale[1.4]"))      //fix text alignment for big CMS
    y0 += .02;
  double y1 = 1.;
...
TString TkAlStyle::header(const PublicationStatus status) {
  ...
  if( status == NO_STATUS ) {
    ...
    txt = "#font[52]{Simulation}";
  } else if( status == PRELIMINARY ) {
    txt = "#scale[1.4]{#font[61]{CMS}} #font[52]{Preliminary}";
  } else if( status == PUBLIC ) {
    txt = "#scale[1.4]{#font[61]{CMS}}";
  } else if( status == SIMULATION ) {
    txt = "#scale[1.4]{#font[61]{CMS}} #font[52]{Simulation}";
  } else if( status == UNPUBLISHED ) {
    txt = "#scale[1.4]{#font[61]{CMS}} #font[52]{(unpublished)}";
